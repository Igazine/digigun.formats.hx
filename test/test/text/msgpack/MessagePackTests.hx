package test.text.msgpack;

import digigun.formats.FormatErrorCode;
import digigun.formats.text.msgpack.MessagePackArray;
...
import digigun.formats.text.msgpack.MessagePackReader;
import digigun.formats.text.msgpack.MessagePackValues;
...
class MessagePackTests {
  ...
  static function testMessagePackParsing():Void {
    var input = Bytes.ofHex(FixtureTools.hex("text/msgpack/parse.hex"));
  ...
  static function testMessagePackRoundTrip():Void {
    ...
    Assertions.assertEquals("msgpack serialized fixture", FixtureTools.hex("text/msgpack/serialize.hex"), bytes.toHex().toLowerCase());
  ...
  static function testMessagePackEdgeFixture():Void {
    var input = Bytes.ofHex(FixtureTools.hex("text/msgpack/edge.hex"));
    var reader = new MessagePackReader();
