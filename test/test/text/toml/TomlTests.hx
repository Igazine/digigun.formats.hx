package test.text.toml;

import digigun.formats.FormatErrorCode;
import digigun.formats.text.toml.TomlCodec;
import digigun.formats.text.toml.TomlDocument;
import digigun.formats.text.toml.TomlObject;
import digigun.formats.text.toml.TomlProperty;
import digigun.formats.text.toml.TomlReader;
import digigun.formats.text.toml.TomlTable;
import digigun.formats.text.toml.TomlValue;
import digigun.formats.text.toml.TomlValues;
...
class TomlTests {
  ...
  static function testTomlParsing():Void {
    var source = FixtureTools.text("text/toml/parse.toml");
  ...
  static function testTomlRoundTrip():Void {
    ...
    Assertions.assertEquals("toml serialized fixture", FixtureTools.text("text/toml/serialize.toml"), serialized);
  ...
  static function testTomlEdgeFixture():Void {
    var reader = new TomlReader();
    var source = FixtureTools.text("text/toml/edge.toml");
  ...
  static function testTomlSampleFixture():Void {
    var reader = new TomlReader();
    var source = FixtureTools.text("text/toml/sample.toml");
